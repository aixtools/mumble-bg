...
def activate(request, server_id):
    ...
    try:
        ...
        messages.warning(
            request,
            _('Mumble account created in Cube, but Murmur backup registration sync failed. Requesting a new password later will retry it.'),
        )
    else:
...
def reset_password(request, server_id):
    ...
    try:
        ...
        messages.warning(
            request,
            _('Cube password was reset, but Murmur backup registration sync failed. Resetting again later will retry it.'),
        )
    else:
        if mumble_user.mumble_userid != mumble_userid:
            mumble_user.mumble_userid = mumble_userid
...
def set_password(request, server_id):
    ...
    try:
        ...
        messages.warning(
            request,
            _('Cube password was updated, but Murmur backup registration sync failed. Setting it again later will retry it.'),
        )
    else:
        if mumble_user.mumble_userid != mumble_userid:
            mumble_user.mumble_userid = mumble_userid
    ...
    mumble_users = (
        ...
        .annotate(
            active_session_count=Count(
                'presence_sessions',
                filter=Q(presence_sessions__is_active=True),
                distinct=True,
            ),
...
def toggle_admin(request, mumble_user_id):
    ...
    try:
        ...
        messages.warning(
            request,
            _('Admin status was updated in Cube, but live Murmur session sync failed. Connected users may need to reconnect.'),
        )
