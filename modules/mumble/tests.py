...
from modules.mumble.ice_sync import MumbleSyncError, sync_live_admin_membership
...
from modules.mumble.views import (
    _generate_password,
...
class ComputeDisplayNameTest(TestCase):
    ...
    @patch('modules.mumble.views._get_ticker')
...
class MurmurPulsePersistenceTest(TestCase):
    def setUp(self):
        self.server = _make_server()
    ...
    def _make_state(self, **kwargs):
        ...
        defaults.update(kwargs)
        return _FakePulseState(**defaults)

    def test_upsert_creates_session_and_updates_presence(self):
        observed_at = timezone.now()
        session, created = upsert_session_from_state(
            self.server,
            self._make_state(),
            observed_at=observed_at,
        )
        self.assertTrue(created)
        self.assertEqual(session.mumble_user, self.mu)
        self.assertEqual(session.address, '127.0.0.1')
        self.assertEqual(session.connected_at, observed_at - timedelta(seconds=12))
        self.assertEqual(session.last_spoke, observed_at - timedelta(seconds=3))
        self.mu.refresh_from_db()
        self.assertEqual(self.mu.last_connected, observed_at - timedelta(seconds=12))
        self.assertEqual(self.mu.last_seen, observed_at)
        self.assertEqual(self.mu.last_spoke, observed_at - timedelta(seconds=3))

    def test_disconnect_closes_session_and_updates_user(self):
        observed_at = timezone.now()
        upsert_session_from_state(self.server, self._make_state(), observed_at=observed_at)
        disconnected_at = observed_at + timedelta(seconds=30)
        result = mark_session_disconnected(
            self.server,
            17,
            state=self._make_state(idlesecs=10),
            observed_at=disconnected_at,
        )
        self.assertTrue(result)
        session = MumbleSession.objects.get(server=self.server, session_id=17)
        self.assertFalse(session.is_active)
        self.assertEqual(session.disconnected_at, disconnected_at)
        self.mu.refresh_from_db()
        self.assertEqual(self.mu.last_disconnected, disconnected_at)

    def test_reconcile_closes_missing_sessions(self):
        observed_at = timezone.now()
        upsert_session_from_state(self.server, self._make_state(), observed_at=observed_at)
        stats = reconcile_server_snapshot(self.server, {}, observed_at=observed_at + timedelta(seconds=45))
        self.assertEqual(stats['disconnected'], 1)
        session = MumbleSession.objects.get(server=self.server, session_id=17)
        self.assertFalse(session.is_active)

    def test_reconcile_keeps_local_only_users(self):
        observed_at = timezone.now()
        stats = reconcile_server_snapshot(
            self.server,
            {88: self._make_state(session=88, userid=None, name='monitor')},
            observed_at=observed_at,
        )
        self.assertEqual(stats['created'], 1)
        session = MumbleSession.objects.get(server=self.server, session_id=88)
        self.assertIsNone(session.mumble_user)
        self.assertIsNone(session.mumble_userid)
        self.assertEqual(session.username, 'monitor')

    def test_mark_server_sessions_disconnected(self):
        observed_at = timezone.now()
        upsert_session_from_state(self.server, self._make_state(session=17), observed_at=observed_at)
        upsert_session_from_state(self.server, self._make_state(session=18, userid=None, name='monitor'), observed_at=observed_at)
        disconnected = mark_server_sessions_disconnected(self.server, observed_at=observed_at + timedelta(seconds=5))
        self.assertEqual(disconnected, 2)
        self.assertEqual(MumbleSession.objects.filter(server=self.server, is_active=True).count(), 0)

    def test_record_successful_authentication_updates_auth_timestamp(self):
        record_successful_authentication(self.mu.pk, 'abc123')
        self.mu.refresh_from_db()
        self.assertEqual(self.mu.certhash, 'abc123')
        self.assertIsNotNone(self.mu.last_authenticated)
...
class LiveAdminSyncTest(TestCase):
    ...
    @patch('modules.mumble.ice_sync._open_target_server')
...
class ActivateViewTest(TestCase):
    def setUp(self):
        ...
        self.sync_patcher = patch('modules.mumble.views._sync_remote_registration', return_value=501)
...
class ResetPasswordViewTest(TestCase):
    def setUp(self):
        ...
        self.server = _make_server()
        self.client.force_login(self.user)
        self.sync_patcher = patch('modules.mumble.views._sync_remote_registration', return_value=601)
        self.sync_patcher.start()
        self.addCleanup(self.sync_patcher.stop)
        self.mu = MumbleUser.objects.create(
            user=self.user,
...
class SetPasswordViewTest(TestCase):
    def setUp(self):
        ...
        self.server = _make_server()
        self.client.force_login(self.user)
        self.sync_patcher = patch('modules.mumble.views._sync_remote_registration', return_value=701)
        self.sync_patcher.start()
        self.addCleanup(self.sync_patcher.stop)
        self.mu = MumbleUser.objects.create(
            user=self.user,
...
class DeactivateViewTest(TestCase):
    def setUp(self):
        ...
        self.server = _make_server()
        self.client.force_login(self.user)
        self.unregister_patcher = patch('modules.mumble.views._unregister_remote_registration', return_value=True)
...
class ToggleAdminViewTest(TestCase):
    ...
    @patch('modules.mumble.views._sync_live_admin_membership', return_value=2)
    ...
    @patch('modules.mumble.views._sync_live_admin_membership', side_effect=MumbleSyncError('boom'))
    def test_toggle_admin_keeps_cube_state_when_live_sync_fails(self, mock_sync_live_admin_membership):
        ...
        self.assertIn('Mumble admin granted for Target_User.', messages)
        self.assertIn(
            'Admin status was updated in Cube, but live Murmur session sync failed. Connected users may need to reconnect.',
            messages,
        )
...
class MumbleManagePresenceColumnsViewTest(TestCase):
    ...
    def test_manage_view_exposes_presence_columns(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'Cube Account')
...
class TasksTest(TestCase):
    ...
    def test_update_mumble_groups(self):
        from modules.mumble.tasks import update_mumble_groups
    ...
    def test_update_all_mumble_groups(self):
        from modules.mumble.tasks import update_all_mumble_groups
        with patch('modules.mumble.tasks.update_mumble_groups.delay') as mock_delay:
...
class SyncRegistrationsCommandTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user('testuser', password='pass')
        self.server = _make_server()
        self.mu = MumbleUser.objects.create(
    ...
    @patch('modules.mumble.management.commands.sync_mumble_registrations.sync_mumble_registration', return_value=812)
    def test_sync_command_updates_mumble_userid(self, mock_sync):
        stdout = StringIO()
        call_command('sync_mumble_registrations', stdout=stdout)
        self.mu.refresh_from_db()
        self.assertEqual(self.mu.mumble_userid, 812)
        self.assertIn('legacy_bcrypt_rows=1', stdout.getvalue())


class RunMurmurPulseCommandTest(TestCase):
    @patch('modules.mumble.management.commands.run_murmur_pulse.MurmurPulseService.run')
    def test_run_once_invokes_service(self, mock_run):
        call_command('run_murmur_pulse', '--once', '--poll-interval', '9')
        mock_run.assert_called_once_with(once=True, poll_interval=9)
